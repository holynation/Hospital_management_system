...
class Pharmacy extends CI_Controller{
	...
	public function view_medicine_sold(){
		...
		$each_quantity = '';
		if($result != 'no result'){
			$each_id = explode(",",$result[0]->medicine_id);
			$each_quantity = explode(",",$result[0]->quantity);
		}

		$data['data_sold'] = $result;
		$data['each_medicine_id'] = $each_id;
		$data['each_quantity'] = $each_quantity;
		$data['permission'] = $this->get_role_name();
