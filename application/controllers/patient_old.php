...
class Patient extends CI_Controller {
	...
	public function add_patient(){
		...
		if($this->form_validation->run() === FALSE){
			...
			// upload script
			$upload_path = '';
			if(isset($_FILES['patient_upload'])){
				print_r($_FILES['patient_upload']);
				return;
                        $config['upload_path'] = './assets/img/patient'; 
                        $config['allowed_types'] = 'gif|jpg|png|jpeg';
                        $config['file_ext_tolower'] = false;
                        $config['file_name'] = 'ehm' . "_" . rand(0, 99999) . uniqid('');  
                        $this->load->library('upload', $config);
                        if (!$this->upload->do_upload('patient_upload'))
                        {
                            echo 'File cannot be uploaded';
                            exit;
                            // redirect('/patient/add_patient');
                        }else{
                            $db_image = $this->upload->data();
                            $upload_path = base_url() . 'assets/img/patient/' . $db_image['file_name'];   
                        }
			}else{
				$upload_path = base_url() . 'assets/img/patient/default.jpg';
			}
			...
			$this->session->set_flashdata('success', 'You have successfully added a staff...');
